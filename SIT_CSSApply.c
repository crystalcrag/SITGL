...
static int cssColorHash(DATA8 str, int len)
{
	static uint8_t asso_values[] = {
		  0,  30,   0,  20,   0,  15,  20,   5,  40, 100,
		215,  45, 110,   0,  15, 190,  20,  75,  20, 175,
		200,  35,  95,  10,  60, 115, 255, 255, 255, 255,
		255, 255,   0,  30,   0,  20,   0,  15,  20,   5,
		 40, 100, 215,  45, 110,   0,  15, 190,  20,  75,
		 20, 175, 200,  35,  95,  10,  60, 115,
    };
    uint8_t asso, chr;
	int     hval = len;

	#define decodeState(ref)    ((chr = ref) >= 123 || chr < 65 || (asso = asso_values[chr-65]) == 255 ? 412 : asso)
	switch (hval) {
	default: hval += decodeState(str[6]); // no break;
	case 6:
	case 5:
	case 4:
	case 3: hval += decodeState(str[2]+1); // no break;
	case 2: hval += decodeState(str[1]);
	}
	return hval + decodeState(str[len - 1]);
	#undef decodeState
}
...
{
	static uint8_t wordlist[] = {
		255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 111, 255, 255, 255,
		 33, 132,  79,  40, 255,  29, 255, 109, 255, 255,  43, 255,   8,   5, 116, 255,
		 54,  27,  23,  39, 114,  11,  20, 255,   3,  53, 255, 255,  12,  41, 255,  71,
		255,  22,  59,  52, 108, 255,  31,  17, 117,  16,  18,  14, 107,  61, 255, 255,
		 32,  68,  19,  51, 255,  73,  75, 255, 255, 255,  30,  91, 255, 255, 255,   7,
		 95, 255, 255, 124,   4, 115,   9, 255,  63, 255, 255, 255, 255,  21,  15,   6,
		255, 255,  70,  64,  99, 255, 255, 255,  76, 101, 255, 255,  10, 127, 138, 118,
		 82, 129,  89,  94, 105, 255, 255,  58, 255,  83, 255, 255, 255, 255, 255, 255,
		130,  35,  55, 255, 255,  97, 255, 255, 255, 255, 255,  86,  42,  60, 255, 255,
		255, 255, 255, 255, 255, 121,  45, 136, 255, 255,  47,  98, 255, 255,  81,   1,
		255, 255, 255, 255,  38, 255, 255,  34, 255,  46, 255, 255, 255, 255, 103, 255,
		255, 255,  90, 255, 255, 255,  85, 255,  25, 255, 255, 255,  93,  80,  28, 255,
		255, 255,  87, 255,  65, 255,  66,  96, 100, 123,  56, 255,  78, 255, 255, 255,
		255, 113, 104, 135, 106, 255,  72, 255, 255, 255, 255,  37,  57, 125, 255, 255,
		 77, 102,  44, 110,   2,  84, 255,  74, 255, 255,  67, 255, 255, 255, 112, 126,
		255,  13, 255, 255,  26, 255, 255, 255, 255, 255, 255, 255, 255, 255, 122, 255,
		255, 255, 255,  36, 255, 255, 255, 255,  88, 255, 255, 255, 255,  24,  62, 255,
		 49, 255, 120, 255, 255, 255, 255, 255,  50, 255, 255, 255, 255, 255, 255, 255,
		255, 134, 137, 139, 255, 255, 255,   0, 255, 255, 255, 255, 255, 255, 255, 131,
		255, 255, 255, 255, 255, 119, 255, 255, 255, 255, 255, 128, 255, 255, 255, 255,
		255, 255, 255, 255,  69, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255,
		255,  48, 255, 255, 255, 133, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255,
		255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255,
		255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255,
		255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255,
		255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255,  92,
	};

	int len = strlen(color);
	if (len <= 17 && len >= 3)
	{
		int key = cssColorHash(color, len);

		if (key <= DIM(wordlist) && key >= 0)
		{
			uint8_t off = wordlist[key];

			if (off < 255 && strcasecmp(colorNames[off<<1], color) == 0)
				return (ULONG) colorNames[(off<<1) + 1];
		}
    }
	return 0;
}
...
int cssApply(SIT_Widget node)
{
	...
	#if 0
	if (strcmp(node->tagName, "arrow") == 0)
	{
