...
DLLIMP NVGcolor nvgTransRGBA(NVGcolor c, unsigned char a)
{
	c.a = a / 255.0f;
	return c;
}

DLLIMP NVGcolor nvgTransRGBAf(NVGcolor c, float a)
{
	c.a = a;
	return c;
}
...
DLLIMP void nvgTransformIdentity(float* t)
{
	t[0] = 1.0f; t[1] = 0.0f;
	t[2] = 0.0f; t[3] = 1.0f;
	t[4] = 0.0f; t[5] = 0.0f;
...
DLLIMP void nvgTransformTranslate(float* t, float tx, float ty)
{
	t[0] = 1.0f; t[1] = 0.0f;
	t[2] = 0.0f; t[3] = 1.0f;
	t[4] = tx; t[5] = ty;
...
DLLIMP void nvgTransformScale(float* t, float sx, float sy)
{
	t[0] = sx; t[1] = 0.0f;
	t[2] = 0.0f; t[3] = sy;
	t[4] = 0.0f; t[5] = 0.0f;
...
DLLIMP void nvgTransformRotate(float* t, float a)
{
...
DLLIMP void nvgTransformSkewX(float* t, float a)
{
	t[0] = 1.0f; t[1] = 0.0f;
...
DLLIMP void nvgTransformSkewY(float* t, float a)
{
...
DLLIMP void nvgTransformMultiply(float* t, const float* s)
{
...
DLLIMP void nvgTransformPremultiply(float* t, const float* s)
{
	float s2[6];
...
DLLIMP void nvgTransformPoint(float* dx, float* dy, const float* t, float sx, float sy)
{
...
DLLIMP void nvgStrokeWidth(NVGcontext* ctx, float width)
{
	NVGstate* state = nvg__getState(ctx);
...
DLLIMP void nvgGetCurTextColor(NVGcontext* ctx, NVGcolor * out_color)
{
	...
	memcpy(out_color, &state->fill.innerColor, sizeof *out_color);
}
...
DLLIMP void nvgStrokeColorRGBA8(NVGcontext* ctx, unsigned char rgba[4])
{
	NVGcolor color;
	// Use longer initialization to suppress warning.
	color.r = rgba[0] / 255.0f;
	color.g = rgba[1] / 255.0f;
	color.b = rgba[2] / 255.0f;
	color.a = rgba[3] / 255.0f;
	NVGstate* state = nvg__getState(ctx);
	nvg__setPaintColor(&state->stroke, color);
}
...
DLLIMP int nvgCreateImage(NVGcontext* ctx, const char* filename, int imageFlags)
{
	...
	if (imageFlags & NVG_IMAGE_MASK)
	{
		/* texture will be used as glyph for text rendering */
		img = stbi_load(filename, &w, &h, &n, 1);
		if (img == NULL) return 0;
		image = ctx->params.renderCreateTexture(ctx->params.userPtr, NVG_TEXTURE_ALPHA, w, h, imageFlags, img);
	}
	else if (imageFlags & NVG_IMAGE_GLTEX)
	{
		/* filename is actually a opengl texture id */
		img = NULL;
		image = nvgCreateImageRGBA(ctx, 0, 0, imageFlags, (const unsigned char *) filename);
	}
	else
	{
...
DLLIMP int nvgCreateImageMem(NVGcontext* ctx, int imageFlags, unsigned char* data, int ndata)
{
	...
	if (imageFlags & NVG_IMAGE_MASK)
	{
		img = stbi_load_from_memory(data, ndata, &w, &h, &n, 1);
		if (img == NULL) return 0;
		image = ctx->params.renderCreateTexture(ctx->params.userPtr, NVG_TEXTURE_ALPHA, w, h, imageFlags, img);
	}
	else
...
DLLIMP NVGpaint nvgLinearGradient(NVGcontext* ctx,
...
DLLIMP NVGpaint nvgRadialGradient(NVGcontext* ctx,
...
DLLIMP NVGpaint nvgBoxGradient(NVGcontext* ctx,
...
DLLIMP NVGpaint nvgImagePattern(NVGcontext* ctx,
...
DLLIMP int nvgTextFit(NVGcontext* ctx, const char* string, const char* end, float maxWidth, float * textWidth)
{
	...
	while (fonsTextIterNext(ctx->fs, &iter, &q))
	{
		...
		// Break to new line when a character is beyond break width.
		if (iter.nextx > maxWidth /*&& rowLen > 0*/)
		{
			if (textWidth) *textWidth = rowWidth;
			return rowLen;
		}
		rowWidth = iter.nextx;
		rowLen = iter.next - string;
	}
